...
class RagProcessor:
    ...
    def get_entity_details(self, script_text: str, entity_list: list[str], entity_type: str, mentions_dict: dict = None) -> list[dict]:
        ...
        but sends a single prompt to OpenAI for efficiency.

        Args:
            script_text: The full script content.
            entity_list: A list of normalized entity names.
            entity_type: Either 'character' or 'location'.
            mentions_dict: Optional dict mapping character names to their mention count.

        Returns:
            A list of dictionaries, each with analysis results.
        """
        if not entity_list:
        ...
        if self.vector_store is None:
            # self._build_vector_store(script_text)
